...
$sql = "SELECT order_id";
$sql.=" FROM v_orders WHERE status = '2' ";
...
//$sql.=" FROM v_orders WHERE 1=1 AND status = '2' ";
...
$productid = $productid_state->fetchAll(PDO::FETCH_ASSOC);
	...
	$nestedData[] = "<input type='checkbox'  class='deleteRow_processeds' value='".$row['order_id']."'  />";
    ...
    '.(($produ['code']=='1')?'
			...
			<form class="code-form-latest_processeds" id="'.$row['order_id'].'" method="post">
									<div class="ui form" style="position: relative;">
									
										<input type="text" style="width: 100px; margin-bottom: 5px;" placeholder="Kod (Sparas)" value="'.$row['code'].'" class="code_code_processeds'.$row['order_id'].'" name="code_code">
									
										<div style="position: absolute; right: 10px; top: 10px;" class="form-status-holder_processeds'.$row['order_id'].'"></div>
	...
	':"").'
			...
			<select name="status" class="selectshit_processeds ui dropdown" id="'.$row['order_id'].'">
